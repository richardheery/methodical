...
make_meth_rse_from_bedgraphs = function(bedgraphs, 
  ...
  # Perform setup
  setup = methodical:::make_meth_rse_setup(meth_files = bedgraphs, meth_sites = meth_sites, sample_metadata = sample_metadata, 
  ...
  meth_sites_df = methodical:::split_bedgraphs_into_chunks(bedgraphs = bedgraphs, 
  ...
  methodical:::write_chunks_to_hdf5(temp_chunk_dirs = setup$temp_chunk_dirs, files_in_chunks = setup$files_in_chunks, 
    hdf5_sink = setup$hdf5_sink, hdf5_grid = setup$hdf5_grid)
  
  # Create a RangedSummarizedExperiment
  rse = methodical:::create_meth_rse_from_hdf5(hdf5_filepath = setup$hdf5_filepath, hdf5_dir = hdf5_dir,
...
make_meth_rse_from_array_files = function(array_files, probe_name_column = 1, beta_value_column = 2, 
  ...
  # Perform setup
  setup = methodical:::make_meth_rse_setup(meth_files = array_files, meth_sites = probe_ranges, sample_metadata = sample_metadata, 
    hdf5_dir = hdf5_dir, dataset_name = dataset_name, overwrite = overwrite, chunkdim = chunkdim, 
    temporary_dir = temporary_dir, ...)
  
  # Read in array files and write data from chunks to appropriate temporary directory
  probe_sites_df = methodical:::split_meth_array_files_into_chunks(array_files = array_files, probe_name_column = probe_name_column, 
  ...
  # Write the chunks to the HDF5 file
  methodical:::write_chunks_to_hdf5(hdf5_sink = setup$hdf5_sink, hdf5_grid = setup$hdf5_grid, 
