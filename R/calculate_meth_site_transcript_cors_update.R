...
#' @param transcript_values_list A list with expression values for transcripts.
#' @param tss_for_chunk_list A list of GRanges with the TSS for the current chunk.
#' @param tss_grl_expanded_chunk A GRangesList with the regions associated with each transcript.
...
.tss_iterator <- function(meth_values_chunk, tss_region_indices_list, transcript_values_list, tss_for_chunk_list, 
  tss_grl_expanded_chunk, cor_method, add_distance_to_region, results_dir){
  
  n <- length(tss_for_chunk_list)
  i <- 0L
  function() {
    if(i >= n)
      NULL
      ...
      list(
        ...
        transcript_region = tss_grl_expanded_chunk[[i]],
        transcript_tss = tss_for_chunk_list[[i]], 
        transcript_name = names(tss_for_chunk_list)[i],
...
#' expression values for transcripts, a GRangesList for the transcript and the name of the transcript. 
...
.tss_correlations <- function(correlation_objects){
  ...
  transcript_region <- correlation_objects[["transcript_region"]]
    ...
    transcript_meth_site_cors <- tryCatch({
      transcript_meth_site_cors <- methodical::rapidCorTest(
        ...
        if(add_distance_to_region){
          transcript_meth_site_cors$distance_to_tss <- 
            methodical::strandedDistance(query_gr = GenomicRanges::GRanges(transcript_meth_site_cors$meth_site), 
              subject_gr = transcript_tss)
          transcript_meth_site_cors$transcript_region = transcript_region$region[subjectHits(findOverlaps(GRanges(transcript_meth_site_cors$meth_site), transcript_region))]
        }
...
calculateMethSiteTranscriptCors <- function(meth_rse, assay_number = 1, transcript_expression_table, 
  samples_subset = NULL, tss_grl, expand_upstream = 5000, expand_downstream = 500, cor_method = "pearson", 
  ...
  stopifnot(is(meth_rse, "RangedSummarizedExperiment"), is(assay_number, "numeric"),
    ...
    is(tss_grl, "GRangesList"), is(expand_upstream, "numeric"), is(expand_downstream, "numeric"),
  ...
  # Check that all regions in tss_grl have names
  if(is.null(names(tss_grl))){stop("names(tss_grl) should be the names of the transcripts associated with each TSS and cannot be NULL")}
     
  # Check that there are no duplicated transcript names in tss_grl or transcript_expression_table
  if(anyDuplicated(names(tss_grl)) | anyDuplicated(row.names(transcript_expression_table))){
    stop("There cannot be duplicated transcript names in names(tss_grl) or row.names(transcript_expression_table)")
  }
  
  # Identify transcripts in common between tss_grl and transcript_expression_table.
  # Throw an error if there are no common transcripts and subset tss_grl and transcript_expression_table
  common_transcripts <- intersect(names(tss_grl), row.names(transcript_expression_table))
  if(length(common_transcripts) == 0){
    stop("There are no common transcripts/genes between names(tss_grl) and row.names(transcript_expression_table)")
  } else {
    message(paste("There are", length(common_transcripts), "genes/transcripts in common between tss_grl and transcript_expression_table"))
  }
  transcript_expression_table <- transcript_expression_table[common_transcripts, , drop = FALSE]
  tss_grl <- tss_grl[common_transcripts]
  ...
  # Expand tss_grl and subset meth_rse for these regions
  tss_grl_expanded = .expand_transcript_ranges(tss_grl, expand_upstream, expand_downstream)
  
  # Get TSS for each transcript
  tss_sites = resize(unlist(reduce(tss_grl)), width = 1, fix = "start")
  
  # Convert tss_grl_expanded to a GRanges
  tss_gr_expanded_reduced = unlist(reduce(tss_grl_expanded))
  
  # Split tss_gr_expanded_reduced into chunks based on the number of methylation sites that they cover
  genomic_region_bins <- methodical:::.chunk_regions(meth_rse = meth_rse, genomic_regions = tss_gr_expanded_reduced, 
  ...
  all_correlations <- foreach::foreach(chunk = seq_along(genomic_region_bins)) %do% {
    ...
    tss_grl_expanded_chunk <- tss_grl_expanded[names(regions_for_chunk)]
    tss_for_chunk <- tss_sites[names(regions_for_chunk)]
    ...
    # Create lists with the transcript values and TSS for each transcript
    transcript_values_list <- lapply(names(tss_region_indices_list), function(x) transcript_expression_table_chunk[x, , drop = FALSE])
    tss_for_chunk_list <- split(tss_for_chunk, names(tss_for_chunk))[names(tss_region_indices_list)] 
    ...
    tss_iter <- .tss_iterator(meth_values_chunk, tss_region_indices_list, transcript_values_list, 
      tss_for_chunk_list, tss_grl_expanded_chunk, cor_method, add_distance_to_region, results_dir)
    ...
    chunk_correlations <- setNames(chunk_correlations, names(tss_for_chunk_list))
