#' Correct methylation values or gene expression values for covariates
#' 
#' Corrects for covariates by regressing out their effects using linear models. 
#' 
#' @param values A table with methylation values or expression values to correct
#' @param covariates Either a data.frame with covariates to correct values for or a design 
#' matrix constructed with model.matrix(). Row names should match column names of values.
#' @return A matrix with the same dimensions of values containing the methylation or expression values 
#' corrected for the covariates. 
#' @export
#' @examples 
adjust_covariates = function(values, covariates){
  
  # Check that the column names of values equal the row names of covariates
  if(any(colnames(values) != row.names(covariates)) | is.null(row.names(covariates))){
    stop("Row names of covariates must match column names of values")
  }
  
  # If covariates is a data.frame, create a design matrix from it otherwise assume that 
  # covariates is already a design matrix
  if(is(covariates, "data.frame")){
    design <- model.matrix(as.formula(paste("~", paste(colnames(covariates), collapse = " + "))), data = covariates)
  } else {
    design <- covariates
  }
  
  # Remove any samples from values that had missing values for covariates
  if(any(!colnames(values) %in% row.names(design))){
    warning("Some samples are missing covariate values. These samples are being removed")
    values = values[, row.names(design)]
  }
...
#' @param meth_values_chunk_corrected A table with corrected methylation values for current chunk
...
#' @param transcript_values_corrected A list with corrected expression values for transcripts.
...
#' @param n_covariates Number of covariates to correct for
#' @return An iterator function which returns a list with the parameters necessary for .tss_correlations. 
.tss_iterator_partial_cors = function(meth_values_chunk, meth_values_chunk_corrected,
  ...
  function() {
    if(i >= n)
      ...
      list(
        ...
        meth_table_corrected = meth_values_chunk_corrected[tss_region_indices_list[[i]], , drop = FALSE], 
        transcript_table = transcript_values[[i]],
        transcript_table_corrected = transcript_values_corrected[[i]],
...
#'
#' @param correlation_objects_corrected A list with a table of methylation values, 
...
.tss_partial_correlations = function(correlation_objects){
      
  meth_table <- correlation_objects[["meth_table"]]
  meth_table_corrected <- correlation_objects[["meth_table_corrected"]]
  transcript_table <- correlation_objects[["transcript_table"]]
  transcript_table_corrected <- correlation_objects[["transcript_table_corrected"]]
  ...
  meth_table_corrected <- t(meth_table_corrected)
  ...
  transcript_table_corrected <- setNames(data.frame(t(transcript_table_corrected)), transcript_name)
  ...
  transcript_meth_site_cors <- tryCatch({
    ...
    transcript_meth_site_cors_corrected = dplyr::transmute(transcript_meth_site_cors_corrected, partial_cor = cor, partial_p_val = p_val)
    transcript_meth_site_cors = cbind(transcript_meth_site_cors, transcript_meth_site_cors_corrected)
...
#' @param covariates Either a data.frame with values for covariates or a design matrix. In either case, row.names should be the names of samples.  
#' @param samples_subset Sample names used to subset meth_rse, transcript_expression_table and covariates for. 
#' Provided samples must be found in meth_rse, transcript_expression_table and covariates.
...
#'   
calculateMethSiteTranscriptPartialCors <- function(meth_rse, assay_number = 1, transcript_expression_table, covariates,
  ...
  stopifnot(is(meth_rse, "RangedSummarizedExperiment"), is(assay_number, "numeric"),
    ...
    is(covariates, "data.frame") | is(covariates, "matrix"),
  ...
  # Check that samples_subset are in meth_rse, transcript_expression_table and covariates
  if(!is.null(samples_subset)){
    ...
    if(any(!samples_subset %in% row.names(covariates))){
      stop("Some samples_subset are not in covariates")
    } else {covariates = covariates[samples_subset, , drop = F]}
  }
  ...
  if(!all(colnames(meth_rse) == row.names(covariates))){stop(
    "Sample names in meth_rse and covariates do not match")
  }

  # Find samples with complete values for all covariates and filter meth_rse, transcript_expression_table and covariates for these samples
  complete_covariate_samples = row.names(covariates)[complete.cases(covariates)]
  message(paste("There are", length(complete_covariate_samples), "samples with values for all covariates"))
  meth_rse <- meth_rse[, complete_covariate_samples]
  transcript_expression_table <- transcript_expression_table[, complete_covariate_samples]
  covariates <- covariates[complete_covariate_samples, , drop = FALSE]
  ...
  all_correlations <- foreach::foreach(chunk = seq_along(genomic_region_bins)) %do% {
    ...
    transcript_expression_table_chunk_corrected <- adjust_covariates(values = transcript_expression_table_chunk, covariates = covariates)
    ...
    meth_values_chunk_corrected <- adjust_covariates(values = meth_values_chunk, covariates = design)
    gc()
    ...
    row.names(meth_values_chunk_corrected) <- as.character(SummarizedExperiment::rowRanges(meth_rse_for_chunk))
    ...
    transcript_values_corrected <- lapply(names(tss_region_indices_list), function(x) transcript_expression_table_chunk_corrected[x, , drop = FALSE])
