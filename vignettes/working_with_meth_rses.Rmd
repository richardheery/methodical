...
names(hg38_cpg_islands) <- hg38_cpg_islands$id

# Filter for annotation for chr11
hg38_cpg_islands = hg38_cpg_islands[seqnames(hg38_cpg_islands) == "chr11"]
...
cpg_island_methylation <- summarizeRegionMethylation(
  meth_rse = mcrpc_wgbs_hg38_chr11, genomic_regions = hg38_cpg_islands$hg38_cpg_islands, 
...
sample using the `plotRegionValues()` function. We'll demonstrate this 
...
gstp1_methylation_plot = plotRegionValues(gstp1_cpg_methylation, sample_name = "DTB_003")
...
`hg38_cpg_islands` GRangesList we created. 

```{r}
# Annotate gstp1_methylation_plot with hg38_cpg_islands
annotatePlot(meth_site_plot = gstp1_methylation_plot, 
  annotation_grl = hg38_cpg_islands, annotation_plot_proportion = 0.3, 
...
annotatePlot(meth_site_plot = gstp1_methylation_plot, 
  annotation_grl = hg38_cpg_islands, 
...
line_elements_hg38 <- repeat_annotation_hg38[repeat_annotation_hg38$repClass == "LINE"]
...
extractGRangesMethSiteValues(meth_rse = mcrpc_wgbs_hg38_chr11, 
  genomic_regions = line_elements_hg38[912388])[, 1:6]
...
options(timeout = Inf)
chr11_dss <- rtracklayer::import("http://hgdownload.soe.ucsc.edu/goldenPath/hg19/chromosomes/chr11.fa.gz", format = "fasta")

# Get CpG sites for hg19 for chromsome 11
hg19_cpgs <- methodical::extractMethSitesFromGenome(genome = chr11_dss, stranded = FALSE)
...
# 1,286,553 methylation sites could be lifted over from hg38 to hg19. 
dim(mcrpc_wgbs_hg38_chr11)
dim(mcrpc_wgbs_hg19_chr11)

# chr11:67581759 should be lifted over to chr11:67349230 so confirm that they have 
