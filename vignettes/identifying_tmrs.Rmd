...
We'll next create a plot of the correlation values using `plotMethSiteValues` so that we can 
...
tubb6_correlation_plot_chrom_coords <- plotMethSiteValues(tubb6_cpg_meth_transcript_cors, 
...
tubb6_correlation_plot_tss_dist <- plotMethSiteValues(tubb6_cpg_meth_transcript_cors, 
  column_name = "cor", ylabel = "Spearman Correlation", value_colours = "set2", 
...
We'll show the Methodical scores for the TUBB6 CpG correlation values using the `plotMethSiteValues` function.
...
done using the `find_TMRs` function which takes the correlation results from `calculateMethSiteTranscriptCors`
...
`plotMethSiteValues`, we can add a reference_positive so that we can see where the TMRs 
