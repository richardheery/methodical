...
the identification of TMRs for the TUBB6 gene. The process is easily scaled-up to search for
TMRs associated with a large number of TSS of interest using parallel computing.
...
The first step in the identifcation of TMRs is to calculate the correlation values between methylation
of CpG sites close to a TSS and the expression of the associated transcript. We do this using the 
`calculate_meth_site_transcript_cors` function which takes a RangedSummarizedExperiment with DNA 
...
`calculate_meth_site_transcript_cors` returns a list of tables with the correlation results 
...
cpg_meth_transcript_cors = calculate_meth_site_transcript_cors(meth_rse = tubb6_meth_rse, 
  transcript_expression_table = tubb6_transcript_counts, tss_gr = tubb6_tss, expand_upstream = 5000, expand_downstream = 5000)

# Since cpg_meth_transcript_cors is just a list of with 1 table, we'll extract this table from the list 
...
We'll next create a plot of the correlation values using `plot_meth_site_values` so that we can 
...
tubb6_correlation_plot_chrom_coords = plot_meth_site_values(tubb6_cpg_meth_transcript_cors, 
  column_name = "cor", ylabel = "Spearman Correlation", low_colour = "#7B5C90", high_colour = "#bfab25") +
...
tubb6_correlation_plot_tss_dist = plot_meth_site_values(tubb6_cpg_meth_transcript_cors, 
  column_name = "cor", ylabel = "Spearman Correlation", low_colour = "#7B5C90", high_colour = "#bfab25", 
  reference_region = attributes(tubb6_cpg_meth_transcript_cors)$tss_range) +
  geom_hline(yintercept = 0, linetype = "dashed")
...
We'll show the Methodical scores for the TUBB6 CpG correlation values using the `plot_meth_site_values` function.

```{r, eval=TRUE}
# Plot methodical scores for CpGs near TUBB6 TSS
tubb6_methodical_scores_plot = plot_methodical_scores(meth_site_values = tubb6_cpg_meth_transcript_cors, 
  p_value_threshold = NULL, smooth_scores = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed")
...
tubb6_smoothed_methodical_scores_plot = plot_methodical_scores(meth_site_values = tubb6_cpg_meth_transcript_cors, 
  p_value_threshold = NULL, smooth_scores = TRUE, smoothed_curve_colour = "hotpink2", offset_length = 10, smoothing_factor = 0.75) +
  geom_hline(yintercept = 0, linetype = "dashed")
...
tubb6_smoothed_methodical_scores_plot = plot_methodical_scores(meth_site_values = tubb6_cpg_meth_transcript_cors, 
  p_value_threshold = 0.005, smooth_scores = TRUE, smoothed_curve_colour = "hotpink2") +
  geom_hline(yintercept = 0, linetype = "dashed")
print(tubb6_smoothed_methodical_scores_plot)
```

We can see two regions where the Methodical scores breach the thresholds: one region around 
12,305,000 where the positive threshold is breahced and another just before 12,307,500 where the 
...
done using the `find_tmrs` function which takes the correlation results from `calculate_meth_site_transcript_cors`
as input. We can set the p-value threshold as well as the parameters for the smoothing,
`offset_length` and `smoothing_factor`, which behave the same as they do in `plot_methodical_scores`.
...
`find_tmrs` returns a GRanges with metadata columns giving the TMR direction, the name of the TMRs,
...
tubb6_tmrs = find_tmrs(correlation_df = tubb6_cpg_meth_transcript_cors, offset_length = 10, smoothing_factor = 0.75, 
  min_meth_sites = 5, min_gapwidth = 150)
print(tubb6_tmrs)
```

We can add these TMRs to either the plot of correlation values or Methodical scores using
`plot_tmrs` to visualize where they are located in relation to these values. We'll 
plot the TMRs on the Methodical scores first and then the correlation values. As with
`plot_meth_site_values`, we can add a reference_positive so that we can see where the TMRs 
...
plot_tmrs(meth_site_plot = tubb6_smoothed_methodical_scores_plot, tmrs_gr = tubb6_tmrs)

# Show location of TMRs on correlation value plot
plot_tmrs(meth_site_plot = tubb6_correlation_plot_tss_dist, tmrs_gr = tubb6_tmrs,
  reference_region = GRanges(tubb6_tmrs$tss_location[1]))
```
