...
```{r, eval=TRUE}
# delete
mcrpc_wgbs_hg38_rse = HDF5Array::loadHDF5SummarizedExperiment("~/wgbs/temp/mcrpc_wgbs_hg38_chr11/")
...
system.time({transcript_body_meth_cors_mcrpc_samples_5kb = calculate_region_methylation_transcript_cors(
  meth_rse = mcrpc_wgbs_hg38_rse, 
  transcript_expression_table = mcrpc_transcript_counts_normalized, 
  samples_subset = common_mcprc_samples, 
...
head(transcript_body_meth_cors_mcrpc_samples_5kb)
```

## Conclusion

Within just a few minutes, we calculated all the relevant correlations and their 
p-values for TSS on chromosome 11. Thus, in under an hour we could easily 
analyse the correlations for TSS in the genome on a desktop computer or even a 
laptop using just a few cores. 
