...
mcrpc_wgbs_hg38_chr11 <- downloadMethDataset(dataset = "mcrpc_wgbs_hg38_chr11")

# Download transcript counts
download.file("https://zenodo.org/record/8422703/files/mcrpc_transcript_counts.tsv.gz", 
  destfile = paste(tempdir(),"mcrpc_transcript_counts.tsv.gz", sep = "/"))
mcrpc_transcript_counts <- data.frame(data.table::fread(paste(tempdir(),"mcrpc_transcript_counts.tsv.gz", sep = "/")), row.names = 1)
```
...
`calculateMethSiteTranscriptCors` function, which takesa methylation 
...
system.time({transcript_cpg_meth_cors_mcrpc_samples_5kb <- calculateMethSiteTranscriptCors(
  ...
  expand_upstream = 5000, expand_downstream = 5000, BPPARAM = BiocParallel::bpparam())})
```
```
...
changes around the TSS. We can use the `plotMethSiteValues` function to 
...
meth_cor_plot_absolute_distance <- plotMethSiteValues(
...
meth_cor_plot_relative_distance <- plotMethSiteValues(
  meth_site_values = gstp1_cpg_meth_transcript_cors, 
  column_name = "cor", 
  reference_tss = TRUE,   
  ylabel = "Correlation Value", 
